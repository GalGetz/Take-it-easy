...
class MCTSNode:
    ...
    def best_child(self, base_exploration_weight=1.0):
        # Determine the number of tiles placed
        tiles_placed = len(self.state.tiles) - len(self.state.get_empty_tiles())

        # Adjust the exploration term based on the number of tiles placed
        max_tiles = 19  # Hardcoded value for maximum tiles
        adjusted_exploration_weight = base_exploration_weight * (1 - (tiles_placed / max_tiles))
        # UCB1 formula with the adjusted exploration term
        choices_weights = [
            (child.value / child.visits) + adjusted_exploration_weight * math.sqrt(2 * math.log(self.visits) / child.visits)
